...
__global__ void SoftmaxWarpImpl(LOAD load, STORE store, const int64_t rows, const int64_t cols) {
  ...
  // 获取全局的线程组id = 线程在block内所在组 + 4 * blockIdx.x
  const int global_thread_group_id = blockIdx.x * blockDim.y + threadIdx.y;
  // 获取全局线程组的数量 = block数量 * blockDim.y（=4）
...
#pragma unroll
    for (int row_id = 0; row_id < rows_per_access; ++row_id) {
      // 把 thread_max[row_id] 初始化为 -inf负无穷
...
// 根据不同的条件来调用不同的 LaunchSoftmaxWarpImpl 函数，以实现对 Softmax 操作的调度
template<typename LOAD, typename STORE, typename ComputeType, int pack_size, int cols_per_thread,
...
inline cudaError_t DispatchSoftmaxWarpImplPadding(cudaStream_t stream, LOAD load, STORE store, const int64_t rows, const int64_t cols) {
  ...
  if (cols == cols_per_thread * thread_group_width) {
    // 如果不需要填充，调用 LaunchSoftmaxWarpImpl 并传递 false 作为最后一个参数，表示不使用填充。
    ...
    // 如果需要填充，调用同样的 LaunchSoftmaxWarpImpl，但传递 true 作为最后一个参数，表示使用填充。
    return LaunchSoftmaxWarpImpl<LOAD, STORE, ComputeType, pack_size, cols_per_thread,
...
// 根据输入的列数和其他参数，动态选择合适的 Softmax 实现版本
// pack_size 这里必须为 1，因为使用了 std::enable_if<pack_size == 1> 来约束该函数仅在 pack_size 为 1 时可用。
...
// 这个宏接受一个参数 thread_group_width，并根据 cols 和 thread_group_width * pack_size 的比较决定调用哪个 DispatchSoftmaxWarpImplPadding 函数。
// 如果列数 cols 小于等于 thread_group_width * pack_size，会进一步检查行数 rows 是否为偶数：
// 如果是偶数，则调用 DispatchSoftmaxWarpImplPadding，并设置 rows_per_access 为 2。
// 如果是奇数，则将 rows_per_access 设置为 1。
#define DEFINE_ONE_ELIF(thread_group_width)                                                        \
  ...
  // 调用：这些调用相当于将不同的线程组宽度传递给宏，以便进行条件判断。将宏定义展开即是不同的else if。
  DEFINE_ONE_ELIF(1)
  DEFINE_ONE_ELIF(2)
  DEFINE_ONE_ELIF(4)
  DEFINE_ONE_ELIF(8)
...
#define DEFINE_ONE_ELIF(col)                                                                      \
    ...
    return DispatchSoftmaxWarpImplPadding<LOAD, STORE, ComputeType, pack_size, col,               \
                                          kWarpSize, 1, algorithm>(stream, load, store, rows, cols);\
  }
  DEFINE_ONE_ELIF(2)
  DEFINE_ONE_ELIF(3)
  DEFINE_ONE_ELIF(4)
...
// 定义了一个模板结构体, 根据输入的列数（cols）来选择合适的pack_size大小，将任务分派给相应的实现函数
...
int main(){
  ...
  // 使用 DefaultComputeType 模板来确定计算时所使用的数据类型
  ...
  // 创建 DirectLoad 对象 load，负责从 input_device 中加载数据，使用列数 cols
  DirectLoad<float, ComputeType> load(input_device, cols);

  // 为输出分配内存，创建 DirectStore 对象 store，用于将计算结果存储到 output_device。
