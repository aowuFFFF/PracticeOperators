...
__global__ void histogram_gmem_atomics(char* data, int n, int *out, int n_bins);
__global__ void histogram_smem_atomics(char* data, int n, int* histo, int n_bins);
...
int main(int argc, char* argv[])
{
    ...
    if (whichKernel == 0) {
        ...
        histogram_gmem_atomics<<<blocks, threads>>>(d_data, n, d_double_kernel_temp_histo, 7);
        cudaDeviceSynchronize();
        histogram_final_accum<<<blocks, threads>>>(d_double_kernel_temp_histo, n_bins*blocks, d_histo, 7);
        cudaMemcpy(h_histo, d_histo, n_bins*sizeof(int), cudaMemcpyDeviceToHost);

    }
    else if (whichKernel == 5) {
        histogram_smem_atomics<<<blocks, threads>>>(d_data, n, d_double_kernel_temp_histo, 7);
        cudaDeviceSynchronize();
        histogram_final_accum<<<blocks, threads>>>(d_double_kernel_temp_histo, n_bins*blocks, d_histo, 7);
        cudaMemcpy(h_histo, d_histo, n_bins*sizeof(int), cudaMemcpyDeviceToHost);
    }
...
__global__ void histogram_gmem_atomics(char* data, int n, int *out, int n_bins)
{
  ...
  for (int i = tid; i < n; i += blockDim.x*gridDim.x) { 
      int alphabet_pos = data[i] - 'a';
      if (alphabet_pos >= 0 && alphabet_pos < 26)
            atomicAdd(&gmem[alphabet_pos/4], 1);
    }
}
